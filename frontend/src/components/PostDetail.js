...
export default function PostDetail({ post, setShowPostDetail }) {
  const { instance, loggedUser, isLoading, setIsLoading } = useContext(AuthContext);
  ...
  const userLikedPost = singlePost?.likes?.some(element => {
    if (element.userId === loggedUser.id) {
      return true;
    }
    return false;
  });
  ...
  const toggleLike = () => {
    ...
    if (!userLikedPost) {
      instance
        .post(`post/toggleLike`, { postId: postId, userId: loggedUser.id })
        .then((res) => {
          toast.success('Liked!');
        })
  ...
  return (
    <div className="absolute top-0 left-0 z-50">
      <div className="fixed z-50 flex items-center justify-center w-screen h-screen px-6 py-14 bg-black/50">
            ...
            <div className="flex flex-col justify-between w-full p-2 font-semibold text-gray-600 border-gray-200 md:ml-4 md:h-full md:border-l md:w-1/4">
              <div className="flex flex-col justify-between h-full">
                {!postByLoggedUser ? (
                  ...
                  <div className="flex justify-center text-xl text-red-500 drop-shadow-lg pb-7 h-1/3 md:flex-col">© your_post</div>
                )}
